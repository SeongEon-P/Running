...
const RecruitRead = () => {
  ...
  useEffect(() => {
    // 모집 글 데이터 가져오기
    axios
      .get('http://localhost:8080/recruit/read', { params: { rno } })
      .then((response) => {
    ...
    // 신청 인원 데이터 가져오기
    axios
      .get('http://localhost:8080/apply/count', { params: { rno } })
      .then((response) => {
    ...
    // 신청자 리스트 가져오기
    axios
      .get('http://localhost:8080/apply', { params: { rno } })
      .then((response) => {
  ...
  const handleDeleteClick = () => {
    if (window.confirm('삭제하시겠습니까?')) {
      axios
        .delete(`http://localhost:8080/recruit/${rno}`)
        .then((response) => {
          alert('게시글이 삭제되었습니다.');
  ...
  const handleApplyClick = () => {
    ...
    axios
      .post('http://localhost:8080/apply', appliedDTO)
      .then((response) => {
        alert('신청이 완료되었습니다.');
  ...
  const handleCancelApplication = (ano) => {
    if (window.confirm('신청을 취소하시겠습니까?')) {
      axios
        .delete(`http://localhost:8080/apply/${ano}`)
        .then((response) => {
          alert('신청이 취소되었습니다.');
